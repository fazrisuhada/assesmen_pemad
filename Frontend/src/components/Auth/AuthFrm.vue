                ...
                <form @submit.prevent="handleSubmit">
                    <div class="flex flex-col px-8 py-8 gap-6 rounded-2xl" style="background-image: radial-gradient(circle at left top, var(--p-primary-400), var(--p-primary-700))">
                        ...
                        <div class="inline-flex flex-col gap-2">
                            <label for="user_name" class="text-primary-50 font-semibold">User Name</label>
                            <InputText 
                                id="user_name" 
                                type="text" 
                                v-model="input.user_name" 
                                class="!bg-white/20 !border-0 !p-4 !text-primary-50 w-80"
                                :class="{ '!border-red-500 !border-2': authStore.errors.user_name }"
                                @input="clearFieldError('user_name')"
                            />
                            <MessageComponent 
                                v-if="authStore.errors.user_name" 
                                :message="authStore.errors.user_name"
                            />
                        </div>
